...
export function renderIntelligenceBrain() {
  ...
  el.innerHTML = `
    ...
    ${selected ? `<div class="gp-card gp-brain-details">
      ...
      <div style="margin-top:11px;font-size:11px;font-weight:700">Relationships (${selectedEdges.length}${edges.filter(e => String(e.source)===selectedId || String(e.target)===selectedId).length>12?'+':''})</div>
      ${selectedEdges.map(e=>{
  ...
  el.querySelector('#brainClearSelection')?.addEventListener('click',()=>{selectedId=null;showAllEvidence=false;window.dispatchEvent(new CustomEvent('gp:brain-select',{detail:{id:null,source:'brain'}}));renderIntelligenceBrain();});
