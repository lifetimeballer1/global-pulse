...
function unique(points){const seen=new Set();return points.filter(p=>{const key=String(p.id??p.eventId??p.mapId??p.datasetEventId??p.sourceUrl??p.url??`${p.__lat.toFixed(4)},${p.__lon.toFixed(4)},${p.title??p.name??p.location??''}`);if(seen.has(key))return false;seen.add(key);return true})}
function collect(){
...
export async function loadMapData(){const [snapshot,events,regional,cartel,links,points]=await Promise.all(['snapshot','mapEvents','mapRegional','mapCartel','mapLinks','mapPoints'].map(getFeed));mapData={snapshot,events,regional,cartel,links,points};renderMap();return mapData}
export function renderMap(){if(!map)initMap();if(!map)return;for(const g of Object.values(groups))g.clearLayers();const all=collect();const points=all.filter(matches).slice(0,5000);const counts=Object.fromEntries(Object.keys(LAYERS).map(k=>[k,0]));for(const p of points){const k=classify(p);counts[k]++;const m=LAYERS[k];const marker=L.circleMarker([p.__lat,p.__lon],{radius:13,color:'#ffffff',weight:3,fillColor:m.color,fillOpacity:1,opacity:1,interactive:true});marker.bindTooltip(String(p.title||p.name||p.location||m.label).slice(0,120),{direction:'top',sticky:true});marker.on('click',e=>{e.originalEvent?.stopPropagation();showDetail(p)});groups[k].addLayer(marker)}const total=points.length;const count=document.getElementById('gpMapCount');if(count)count.textContent=`${total.toLocaleString()} signals`;for(const k of Object.keys(LAYERS)){const e=document.getElementById(`gpMapLayerCount-${k}`);if(e)e.textContent=counts[k].toLocaleString()}if(total)fitAll(points);else setTimeout(()=>map.invalidateSize(),50)}
