...
The Brain is intentionally small. Raw news, map observations, events, claims,
assessments and market records stay upstream as evidence. The Brain exposes only
major, source-backed things a human should see as nodes: countries, major
cartels/organizations, economic factors, and a small number of major strategic
conflict/chokepoint nodes. Never create a node without source evidence.
"""
...
ROOT=Path(__file__).resolve().parent;DATA=ROOT/'data';OUT=DATA/'intelligence_brain.json';MAX_NODES=40
...
def main():
 ...
 # Canonical nodes: only major entities/factors, never raw reports.
 ...
 # Market data is evidence for the single Markets economic factor, not dozens of market nodes.
 ...
 for q in market.get('indicators') or []:
  if isinstance(q,dict) and q.get('price') is not None:
   market_sources.append({'title':str(q.get('name') or q.get('symbol') or 'Market indicator'),'url':str(q.get('sourceUrl') or q.get('url') or ''),'source':str(q.get('source') or 'Yahoo Finance public market feed'),'time':str(q.get('marketTime') or q.get('updatedAt') or '')})
 if market_sources:
  mid=add('Markets','economic',market_sources[0],4,{'group':'Economic Factor','canonical':True,'marketIndicators':len(market_sources)})
  if mid:
   nodes[mid]['evidence'].extend([x for x in market_sources[1:] if not any(y.get('title')==x.get('title') and y.get('url')==x.get('url') for y in nodes[mid]['evidence'])])
 # Select only the strongest canonical hubs. Prefer sourced cartels, then economic
 # factors, then countries, then the few strategic categories.
 priority={'cartel':0,'country':1,'economic':2,'conflict':3,'chokepoint':4}
 quotas={'cartel':16,'country':10,'economic':10,'conflict':2,'chokepoint':2}
 chosen=[]
