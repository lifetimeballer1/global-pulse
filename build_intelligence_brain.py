...
def ev(r):
 ...
 t=str(r.get('title') or r.get('name') or r.get('headline') or r.get('label') or '').strip();u=str(r.get('original_link') or r.get('url') or r.get('sourceUrl') or r.get('source_url') or r.get('link') or '').strip();s=str(r.get('sourceLabel') or r.get('source') or r.get('publisher') or r.get('provider') or '').strip()
...
def main():
 ...
 for r,typ,source in reports:
  ...
  for i,a in enumerate(hits):
   for b in hits[i+1:]:link(a,b,'These major intelligence entities/factors are co-mentioned in the same sourced record; this is contextual evidence, not proof of causation.',source,typ)
 if market_sources:
