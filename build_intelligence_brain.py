...
Raw intelligence stays in the source artifacts. The Brain is a consolidated
analysis layer containing canonical country, organization, domain and market
hubs. It is capped at 150 nodes for a readable mobile intelligence web.
No source/evidence means no node or relationship. Links are contextual only.
"""
...
ROOT=Path(__file__).resolve().parent; DATA=ROOT/'data'; OUT=DATA/'intelligence_brain.json'; MAX_NODES=150
...
def main():
 ...
 # Existing graph contributes only source-backed entity candidates.
 graph=load('intelligence_graph.json',{}) or {}
 for n in graph.get('nodes',[]) if isinstance(graph,dict) else []:
  if not isinstance(n,dict):continue
  es=[x for x in n.get('evidence',[]) if isinstance(x,dict) and (x.get('url') or x.get('source'))]
  if es:add(n.get('label') or n.get('id'),'entity',es[0],max(1,int(n.get('mentions',1))))
 for n in nodes.values():n['sourceCount']=len(n['evidence']);n['score']+=min(20,n['sourceCount']*2)+(10 if n.get('canonical') else 0)
 selected=sorted(nodes.values(),key=lambda n:(n['score'],n['sourceCount'],n['mentions']),reverse=True)[:MAX_NODES];keep={n['id'] for n in selected};edges=[]
 ...
 now=datetime.now(timezone.utc).isoformat().replace('+00:00','Z');stats={'nodes':len(selected),'edges':len(edges),'newsRecords':sum(t=='news' for _,t in reports),'conflictRecords':sum(t=='conflict' for _,t in reports),'marketIndicators':len(market.get('indicators') or []),'sourceBackedCandidates':len(nodes)}
 OUT.write_text(json.dumps({'version':4,'updatedAt':now,'complete':True,'consolidated':True,'maxNodes':MAX_NODES,'sourceArtifacts':['snapshot.json','intelligence_graph.json','event_intelligence.json','claims.json','intelligence_assessment.json','event_market_impact.json','what_changed.json'],'method':'All raw records remain upstream; the Brain consolidates them into at most 150 canonical source-backed hubs. Evidence is aggregated onto hubs.','caution':'Relationships are contextual evidence links and do not prove causation, coordination, intent, or responsibility. Market links are context only.','nodes':selected,'edges':edges,'stats':stats},ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8');print(f'INTELLIGENCE BRAIN: {len(selected)} canonical nodes / {len(edges)} evidence relationships / {len(nodes)} source-backed candidates')
