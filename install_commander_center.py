...
FIX_SRC = 'global_pulse_mission_control_fix.js'
...
# Remove every previous Commander Center block, not just the first one.
s = re.sub(r'\n<div id="'+re.escape(NAV_ID)+r'"[^>]*>.*?</nav>\s*</div>\s*', '\n', s, flags=re.S)
# Remove orphaned navigation blocks left by older installer versions.
s = re.sub(r'\n<nav[^>]*class=["\']cc-nav["\'][^>]*>.*?</nav>\s*', '\n', s, flags=re.S | re.I)
# Also remove stray Commander Center title/list fragments if an earlier run
# stopped halfway through the block.
s = re.sub(r'\n<div[^>]*class=["\']cc-title["\'][^>]*>.*?</div>\s*', '\n', s, flags=re.S | re.I)
...
# Remove any prior Mission Control repair script before adding one.
s = re.sub(r'\n<script[^>]*src=["\']'+re.escape(FIX_SRC)+r'[^>]*>\s*</script>\n?', '\n', s, flags=re.S | re.I)
...
nav = '''<div id="commanderCenter" aria-label="Commander Center">
  <div class="cc-title"><strong>COMMANDER CENTER</strong><span>Mission Control · jump to any intelligence layer</span></div>
  <nav class="cc-nav" aria-label="Mission Control navigation">
...
# Mission Control / Active Conflict Watch repair is loaded last so it can
# correct legacy markup generated by earlier UI installers.
s = s.replace('</body>', '<script src="'+FIX_SRC+'?v=1" defer></script>\n</body>', 1)

INDEX.write_text(s, encoding='utf-8')
print('PASS: Commander Center installed cleanly; duplicate navigation removed; valid Mission Control targets installed; conflict-watch repair installed')
