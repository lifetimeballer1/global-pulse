...
The public GitHub Pages site always gets a server-rendered copy of the latest
repository news export. JavaScript then refreshes that list every 60 seconds.
An optional local FastAPI endpoint can be enabled without being required.
"""
...
def initial_cards(limit: int = 30) -> str:
    ...
    for article in articles[:limit]:
        ...
        cards.append(
            f'''<article class="gp-reporting-card">
  <div class="gp-reporting-source">◉ {source}</div>
  {title_html}
  <time class="gp-reporting-time" datetime="{published}">Published {published or "time unavailable"}</time>
  <p class="gp-reporting-summary">{summary}</p>
  <div class="gp-reporting-source-name">Source: {source}</div>
  {action}
</article>'''
        )

    if not cards:
        return '<div class="gp-reporting-empty">No active reporting is currently available.</div>'
    return "\n".join(cards)
...
def patch_index():
    ...
    html_text, count = re.subn(pattern, reporting_html, html_text, count=1, flags=re.I)
    if count == 0:
        pattern2 = r'<section[^>]*>[\s\S]*?<h2[^>]*>\s*Latest Reporting\s*</h2>[\s\S]*?</section>'
        html_text, count = re.subn(pattern2, reporting_html, html_text, count=1, flags=re.I)
    if count == 0:
        marker = '</main>' if '</main>' in html_text else '</div>\n</body>'
        html_text = html_text.replace(marker, reporting_html + '\n' + marker, 1)

    config = '<script id="gp-live-reporting-config">window.GLOBAL_PULSE_API="";</script>'
    html_text = html_text.replace('</head>', CSS + '\n' + config + '\n</head>', 1)
    html_text = html_text.replace('</body>', '<script src="global_pulse_reporting.js" defer></script>\n</body>', 1)
...
if __name__ == "__main__":
    patch_index()
    print("Update 9 applied: static-first live reporting installed with server-rendered initial data.")
