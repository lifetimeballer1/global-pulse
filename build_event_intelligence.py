...
def region_hits(entities):
 return [{'region':region,'entities':sorted(set(entities)&members)} for region,members in REGIONS.items() if set(entities)&members]
 ...
 for r in reports:
  ...
  for g in groups:
   if len(ts&g['tokens'])/max(1,len(ts|g['tokens']))>=.82:
    g['reports'].append(r);g['tokens']|=ts;break
  else:groups.append({'tokens':ts,'reports':[r]})
 return groups

def quality_for(d,se):
 if not d:return 25
 classes=(se.get('domainClasses') or {}) if se else {}
 # source_evidence stores counts by class; use known domain lists as a conservative proxy.
 if d.endswith('.gov') or d.endswith('.mil') or d in {'un.org','nato.int','iaea.org','who.int','imf.org','worldbank.org','state.gov','defense.gov','whitehouse.gov','congress.gov','europa.eu','gov.uk','gov.ua','kremlin.ru','president.gov.ua'}:return 100
 if d in {'reuters.com','apnews.com','bbc.com','bbc.co.uk','aljazeera.com','france24.com','dw.com','ft.com','bloomberg.com','wsj.com','nytimes.com','washingtonpost.com','theguardian.com','cnn.com','npr.org','pbs.org','cnbc.com','politico.com','economist.com','abcnews.go.com','cbsnews.com','nbcnews.com','skynews.com'}:return 90
 if d in {'crisisgroup.org','acleddata.com','cfr.org','csis.org','iiss.org','sipri.org','reliefweb.int','unhcr.org','ohchr.org','ecfr.eu','rusi.org','bellingcat.com'}:return 82
 if d in {'news.google.com','news.yahoo.com','yahoo.com','msn.com','aol.com','apple.news'}:return 35
 if d in {'x.com','twitter.com','facebook.com','instagram.com','tiktok.com','youtube.com','reddit.com','t.me'}:return 20
 return 55
...
def main():
 ev=load('live_events.json',{'events':[]}); snap=load('snapshot.json',{}); se_all=load('source_evidence.json',{}); consistency=load('event_consistency.json',{})
 consistency_by={x.get('eventId'):x for x in consistency.get('events',[]) if isinstance(x,dict)}
 output=[]
 for e in ev.get('events',[]):
  ...
  for r in reports:
   ...
   if t:times.append(t)
  counts=Counter(domains);unique_domains=sorted(set(domains));groups=reporting_groups(reports);group_count=len(groups)
  se=next((x for x in se_all.get('eventSourceEvidence',[]) if x.get('eventId')==e.get('id')),None) or {}
  non_agg=[d for d in unique_domains if d not in {'news.google.com','news.yahoo.com','yahoo.com','msn.com','aol.com','apple.news','x.com','twitter.com','facebook.com','instagram.com','tiktok.com','youtube.com','reddit.com','t.me'}]
  concentration=max(counts.values(),default=0)/max(1,len(domains))
  avg_quality=round(sum(quality_for(d,se) for d in domains)/len(domains)) if domains else 0
  primary=int(se.get('primarySourceDomains',0));major=int(se.get('majorNewsDomains',0)); high_flags=sum(1 for f in (consistency_by.get(e.get('id'),{}).get('flags') or []) if f.get('severity')=='high')
  moderate_flags=sum(1 for f in (consistency_by.get(e.get('id'),{}).get('flags') or []) if f.get('severity')=='moderate')
  # Deliberately conservative: raw report count cannot overwhelm weak independence.
  score=20 + min(32,group_count*10) + min(24,len(non_agg)*6) + min(14,avg_quality/7) + min(8,primary*4+major*2)
  score -= min(18,max(0,(concentration-.5)*36)) + high_flags*18 + moderate_flags*8
  ...
  g=grade(score,group_count,len(non_agg),high_flags)
  if g=='A':status='well-corroborated'
  elif g=='B':status='moderately corroborated'
  elif g=='C':status='limited corroboration'
  else:status='insufficient / conflicting evidence'
  caveats=[]
 ...
 OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(f'EVENT INTELLIGENCE: {len(output)} events graded with evidence-aware confidence')
