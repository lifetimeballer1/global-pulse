...
ROOT=Path(__file__).resolve().parent;SNAP=ROOT/'data'/'snapshot.json';UA='Mozilla/5.0 (compatible; GlobalPulse/12.0)'
...
def fetch_quote(symbol):
 ...
 for host in ('query1.finance.yahoo.com','query2.finance.yahoo.com'):
  try:
   ...
   candle_ts,price=pairs[-1];previous=meta.get('chartPreviousClose') or meta.get('previousClose') or price;previous=float(previous);change=price-previous;pct=change/previous*100 if previous else 0;state=str(meta.get('marketState') or '').upper();status='live' if symbol=='BTC-USD' or state in {'REGULAR','PRE','POST'} else 'closed'
   return {'price':price,'previousClose':previous,'change':change,'changePercent':pct,'marketTime':datetime.fromtimestamp(candle_ts,tz=timezone.utc).isoformat(),'marketState':state,'currency':meta.get('currency'),'exchange':meta.get('fullExchangeName') or meta.get('exchangeName'),'endpoint':host,'interval':'1m','provider':'Yahoo Finance','quoteSource':'latest intraday candle'}
...
def main():
 ...
 with ThreadPoolExecutor(max_workers=8) as pool:
  ...
  for f in as_completed(futures):
   ...
   if q:q.update({'name':name,'symbol':symbol,'type':kind,'unit':unit,'decimals':dec,'checkedAt':now(),'status':q.get('status')});vals[symbol]=q
   else:
    if symbol in old:
     x=dict(old[symbol]);x['status']='stale';x['lastAttemptedAt']=now();vals[symbol]=x
    errors.append({'symbol':symbol,'error':e})
 indicators=[vals[s] for _,s,*_ in WATCH if s in vals]
 if not indicators:
  raise SystemExit(f'MARKET DATA REFRESH BLOCKED: all {len(WATCH)} public quotes failed; previous snapshot preserved')
