...
def stable_id(prefix:str,*parts:str)->str:return f"{prefix}-{hashlib.sha256('|'.join(parts).encode('utf-8','ignore')).hexdigest()[:16]}"
def article_text(article:dict[str,Any])->str:return " ".join(str(article.get(k) or "") for k in ("title","summary_snippet","summary","description","content")).strip()
...
def main()->int:
 ...
 for article in articles:
  ...
  for i,source_id in enumerate(matched):
   for target_id in matched[i+1:]:
    ...
    pair=sorted((source_id,target_id));key=f'{pair[0]}|mentioned_with|{pair[1]}';semantic_a=f'{source_id}|{kind}|{target_id}';semantic_b=f'{target_id}|{kind}|{source_id}'
 ...
 if errors:
  ...
  for error in errors[:25]:print(f' - {error}')
  return 1
 OUTPUT.write_text(json.dumps(document,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(f'PASS: canonical v3 entities={len(entities)} events={len(events)} relationships={len(relationships)} evidence={len(evidence)} discovered={len(discovered_ids)}');return 0
