...
def main():
 snap=load('snapshot.json',{}); graph=load('intelligence_graph.json',{}); events=load('live_events.json',{}); breaking=load('breaking_news.json',{}); source_ev=load('source_evidence.json',{})
 previous=load('intelligence_assessment.json',{})
 ...
 def score_for(label):
  ...
  return clamp(raw),sorted(factors,key=lambda x:abs(x['delta']),reverse=True)[:6],evidence
 old={str(x.get('entity')):x for x in previous.get('assessments') or []}
 assessments=[]
 for label in names:
  ...
  prior=old.get(label); delta=score-int(prior.get('score',score)) if prior else 0
  assessments.append({'entity':label,'score':score,'level':level,'delta':delta,'trend':'UP' if delta>2 else 'DOWN' if delta<-2 else 'STABLE','factors':factors,'evidenceCount':evidence,'method':'Volume, live-event activity, network exposure, economic signals and de-escalation signals from current public reporting. Indicator only; not a causal forecast.'})
 assessments.sort(key=lambda x:x['score'],reverse=True)
 impact=[]
 for e in (events.get('events') or [])[:30]:
  cat=e.get('category','general'); title=e.get('title','Live event')
  ...
  se=next((x for x in source_ev.get('eventSourceEvidence',[]) if x.get('eventId')==e.get('id')),None)
  impact.append({'eventId':e.get('id'),'title':title,'category':cat,'confidence':e.get('confidence','unknown'),'reportCount':e.get('reportCount',0),'sourceCount':e.get('sourceCount',0),'sourceIndependence':se.get('independence') if se else 'unknown','uniqueDomains':se.get('uniqueDomains',0) if se else 0,'whyThisMatters':{'immediate':implications[0],'shortTerm':implications[1],'mediumTerm':implications[2]},'urls':e.get('urls',[])[:5]})
 ...
 print(f'ASSESSMENT: {len(out["assessments"])} entity indicators / {len(out["eventImpacts"])} event impacts / {len(changed)} material changes')
