...
/* Hide the unfinished brief until it has a proper command-center slot. */#gpBrief{display:none!important}
...
# Replace the generated map CSS so repeated workflow runs never stack versions.
if 'id="gp-map-pro-css"' in s:
    s=re.sub(r'<style id="gp-map-pro-css">.*?</style>',css,s,count=1,flags=re.S)
else:
    s=s.replace('</head>',css+'\n</head>',1)

js = r'''<script id="gp-map-pro-js">
/* Update 2: clean professional conflict / OSINT map layer */
(function(){
  ...
  function removeLegacyControls(){
    ...
    /* Remove duplicate copies of our own controls if a cached/older patch created them. */
    const ours=[...document.querySelectorAll('#gpMapTools')];ours.slice(1).forEach(el=>el.remove());
    const legends=[...document.querySelectorAll('#gpMapLegend')];legends.slice(1).forEach(el=>el.remove());
  }
  ...
  function ensureTools(){
    ...
    if(!tools){
      ...
      map.parentElement.insertBefore(tools,map);
    }
  ...
  function cleanTop(){
    /* Remove only known accidental plain-text injections; never touch normal dashboard copy. */
    document.body.childNodes.forEach(n=>{if(n.nodeType===Node.TEXT_NODE&&/Update 2 map patch prepared|professional intelligence map/i.test(n.nodeValue||''))n.remove()});
  }

  function boot(){ensureTools();cleanTop();updateStatus()}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(boot,250));else setTimeout(boot,250);
})();
</script>'''

# Replace the existing Update 2 script instead of adding another copy.
if 'id="gp-map-pro-js"' in s:
    s=re.sub(r'<script id="gp-map-pro-js">.*?</script>',js,s,count=1,flags=re.S)
else:
    s=s.replace('</body>',js+'\n</body>',1)

p.write_text(s,encoding='utf-8')
print('Clean map patch applied')
