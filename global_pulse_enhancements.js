...
(function(){
  ...
  function applyCore(){
    ...
    if(list){list.innerHTML=conflicts.slice(0,30).map((c,i)=>{const s=Math.max(0,Math.min(100,n(c.activityScore??c.score??c.tension??c.priority)));const sev=String(c.escalation||c.severity||c.status||'MONITOR');return '<article class="ccard" tabindex="0"><span class="tag red">'+esc(sev)+'</span><h3>'+esc(c.name||c.title||c.conflict||('Conflict '+(i+1)))+'</h3><div class="muted">'+esc(c.region||c.location||'Global')+' · '+esc(c.confidence||'OPEN-DATA SIGNAL')+'</div><div class="scoreline"><span>Activity signal</span><b>'+Math.round(s)+'</b></div><div class="track"><div class="fill" style="width:'+s+'%"></div></div><div class="muted" style="margin-top:7px">'+esc(c.recent||'Current reporting signal')+'</div></article>'}).join('')||'<div class="empty">No active conflict records in the current snapshot.</div>'}
